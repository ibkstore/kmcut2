...
pdf_file = sprintf("%s_KMopt_minf_%.2f.pdf",pdf_file,min_fraction)

# Name of the output TXT file
txt_file = basename(file_path_sans_ext(fname))
txt_file = sprintf("%s_KMopt_minf_%.2f.txt",txt_file,min_fraction)

# Name of the output CSV file with low/high sample labels
csv_file = basename(file_path_sans_ext(fname))
csv_file = sprintf("%s_KMopt_minf_%.2f_labels.csv",csv_file,min_fraction)
